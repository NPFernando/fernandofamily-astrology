...
export function RoadmapFeedbackClient() {
  ...
  const [votedFirst, setVotedFirst] = useState(false);
  ...
  }).sort((left, right) => votedFirst ? Number(Boolean(votes[right.id])) - Number(Boolean(votes[left.id])) || left.label.localeCompare(right.label) : 0);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <section className="grid gap-3 md:grid-cols-[1.4fr_1fr_1fr_auto]">
        ...
        <label className="flex items-end gap-2 pb-2 text-sm font-medium"><input type="checkbox" checked={votedFirst} onChange={(event) => setVotedFirst(event.target.checked)} />{dict.roadmap.sortVotedFirst}</label>
      </section>
      <div className="flex flex-wrap items-center gap-3 text-sm opacity-75"><p>{dict.roadmap.itemsShown.replace("{n}", String(filtered.length))} {dict.roadmap.localVoteNote}</p>{hasActiveFilters && <div aria-label={dict.roadmap.activeFilters} className="flex flex-wrap gap-2">{query.trim() && <span className="rounded-full border border-black/10 px-2 py-0.5 dark:border-white/20">{dict.roadmap.activeSearch.replace("{value}", query.trim())}</span>}{category && <span className="rounded-full border border-black/10 px-2 py-0.5 dark:border-white/20">{dict.roadmap.activeCategory.replace("{value}", category)}</span>}{statusFilter && <span className="rounded-full border border-black/10 px-2 py-0.5 dark:border-white/20">{dict.roadmap.activeStatus.replace("{value}", dict.roadmap[statusFilter])}</span>}</div>}{hasActiveFilters && <button type="button" onClick={clearFilters} className="font-semibold text-accent underline">{dict.roadmap.clearFilters}</button>}{Object.keys(votes).length > 0 && <button type="button" onClick={clearVotes} className="font-semibold text-accent underline">{dict.roadmap.clearVotes}</button>}{votesCleared && <p role="status">{dict.roadmap.votesCleared}</p>}</div>
