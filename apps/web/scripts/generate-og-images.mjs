...
// PWA icons are generated by generate-heritage-app-icons.mjs. Use that same
// mark in social cards, but never regenerate it here from a feature poster.
const BRAND_MARK = "public/icons/app/icon-512.png";
const LANDING_POSTER = "public/posters/landing-heritage-v2.webp";
...
async function ogImage() {
  ...
  const mark = await sharp(BRAND_MARK).resize(300, 300, { fit: "contain" }).png().toBuffer();
  const poster = await sharp(LANDING_POSTER).resize(W, H, { fit: "cover" }).png().toBuffer();
  await sharp(poster)
    .composite([
      { input: bg, left: 0, top: 0 },
      { input: mark, left: 850, top: 175 },
    ])
...
async function featureOgImage(feature) {
  ...
  const description = clampWords(resolveKey(en, feature.descriptionKey), 9);
  ...
  <text x="86" y="370" font-family="Noto Sans, DejaVu Sans, sans-serif"
        font-size="28" fill="rgba(255,255,255,0.86)">${esc(description)}</text>
...
console.log("Rendered default OG and per-feature OG images from the heritage app mark");
