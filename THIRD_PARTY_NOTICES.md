...
The bird and activity line icons (`apps/web/components/icons/`), the app
icon, the apple-touch-icon, and the Open Graph share image
(`apps/web/public/og/`, generated by `apps/web/scripts/generate-og-images.mjs`)
are original artwork created for this project — not copied or derived from
any commercial or third-party iconography. The share image renders the
platform name using Noto Sans / Noto Sans Sinhala (SIL OFL 1.1, listed
above).
